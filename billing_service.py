...
def _period_datetime_bounds(period_start: date, period_end: date) -> tuple[datetime, datetime]:
    """
    Convert DATE period into inclusive [start, end] on processed_at,
    implemented as [start_at, next_day_of_end).
    """
...
def generate_invoice_for_period(
    ...
    """
    Build or rebuild an invoice for one account for a given period.

    Formula:
      total = 5$ base
            + 3$ * extra_members
            + 7$ * hours_of_video_usage
    """
    ...
    with Session(engine) as session:
        ...
        if invoice is not None and invoice.status != "draft":
            raise ValueError(
                f"Invoice already exists for {period_start}–{period_end} "
                f"with status {invoice.status}, cannot regenerate"
            )
        ...
        if invoice is None:
            ...
            # Clear existing lines if regenerating
        ...
        # Optional: enforce currency consistency
        if account.currency_code != base_price.currency_code:
            raise ValueError(
                f"Currency mismatch: account {account.currency_code} "
                f"vs pricing {base_price.currency_code}"
            )

        # Members: primary + dependants
        total_members = (
        ...
        # Usage: sum billable minutes in period
        start_at, end_at = _period_datetime_bounds(period_start, period_end)
        total_minutes = (
            session.query(UsageVideo)
            .with_entities(
                (UsageVideo.billable_minutes)
            )
            .filter(
        ...
        minutes_sum = sum((row[0] or Decimal("0")) for row in total_minutes)
        minutes_sum = Decimal(minutes_sum)
        hours = (minutes_sum / Decimal("60")).quantize(
        ...
        # 1) Base membership (always 1 per account)
        ...
        session.add(base_line)
        total_amount += base_line_amount

        # 2) Extra member charges
        if extra_members > 0:
...
def get_month_period(year: int, month: int) -> tuple[date, date]:
    """
    Convenience helper: first_day, last_day for a calendar month.
    Example: get_month_period(2025, 12)
    """

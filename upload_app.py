...
def _sportai_submit(video_url: str, email: str | None = None) -> str:
    ...
    print(f"[SPORTAI] POST {url} | email={email} | video_url={video_url}")
    r = requests.post(url, headers=headers, json=payload, timeout=60)
    if not r.ok:
        raise RuntimeError(f"SportAI submit failed {r.status_code}: {r.text}")
    data = r.json() or {}
    ...
    print(f"[SPORTAI] Submitted OK, task_id={tid}")
    return str(tid)

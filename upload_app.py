...
# ---- Bronze ingest (db_init) ----
from db_init import engine, ingest_all_for_session  # << keep as-is; we only use this after result JSON fetch
...
# ------------------------------------------------------------------------------
# Basic health (for Render)
# ------------------------------------------------------------------------------
@app.get("/health")
def health():
    return jsonify(ok=True, service=os.getenv("SERVICE_NAME", "sportai-api"), ts=datetime.now(timezone.utc).isoformat())

@app.get("/")
def root():
    return jsonify(ok=True, service=os.getenv("SERVICE_NAME", "sportai-api"))

# -------------------------------------------------------
# Safety JSON helpers (unchanged)
# -------------------------------------------------------
def _j(ok=True, **k):
...
@app.get("/ops/source-snippet")
def ops_source_snippet():
    try:
...
# -------------------------------------------------------
# Env / config (unchanged names, kept compatible)
# -------------------------------------------------------
...
# Try both public hostnames (unchanged)
...
S3_BUCKET       = os.getenv("S3_BUCKET", os.getenv("UPLOAD_S3_BUCKET", "")).strip()
S3_PREFIX       = os.getenv("S3_PREFIX", os.getenv("UPLOAD_S3_PREFIX", "incoming")).strip().strip("/")
...
# ---------- BACKGROUND INGEST (unchanged orchestration, with patched core) ----------
...
def _store_raw_payload(conn, *, payload_dict: dict, session_id=None, session_uid=None, doc_type="sportai.result", source=None):
...
def _update_raw_result_session_id(conn, *, session_id: int, session_uid: str | None):
    if not session_uid:
        return
    conn.execute(sql_text("""
        UPDATE raw_result
...
def _do_ingest(task_id: str, result_url: str):
    """
    Background worker: fetch JSON, save raw_result, build Bronze.
    This is the ONLY area changed from your original: we swap in raw_result+Bronze.
    """
    try:
        with engine.begin() as conn:
            _ensure_submission_context_schema(conn)
        ...
        with engine.begin() as conn:
            # 1) save raw payload so it never drops off
            ...
            conn.execute(sql_text("""
                UPDATE submission_context
                   SET session_id = :sid,
                       ...
                       ingest_error = NULL
                 WHERE task_id = :t
            """), {"t": task_id, "sid": session_id})

    except Exception as e:
        # store error but never crash process
        try:
            with engine.begin() as conn:
                _ensure_submission_context_schema(conn)
                conn.execute(sql_text("""
...
def api_task_status():
    ...
    try:
        ...
        # auto-start background ingest when result is ready
...
def ops_sportai_callback():
    ...
    # We keep this route for completeness, but your flow uses polling. We still save raw+Bronze if ever called.
    if not result_url:

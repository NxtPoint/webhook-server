...
CREATE_STMTS = {
    ...
    # -------------------------------- point silver ------------------------------
      ...
      WITH
      -- D1. Constants (use SportAI frame ~13 wide; mid_x=6.5) ---------------------
      const AS (
        SELECT
          13.0::numeric  AS court_w,           -- doubles width units
          23.77::numeric AS court_l,
          6.5::numeric   AS mid_x,             -- 13 / 2
          23.77::numeric / 2 AS mid_y,
          0.50::numeric  AS serve_eps_m,       -- 0.5 m band
      ...
      -- D4. Server position at contact (nearest by ts or s) -----------------------
      pos_at_hit AS (
        SELECT
          s.session_id, s.swing_id,
      ...
      -- D5. Serve flags (strict) + robust XY fallback -----------------------------
      serve_flags_all AS (
        SELECT
      ...
      -- D8. Attach each swing to the latest serve at/preceding it ----------------
      swings_in_point AS (
        SELECT
          s.*,
          sp.point_number_d,
      ...
      SELECT
        ...
        -- error rule: ball speed = 0
        (COALESCE(sn.ball_speed, 0) = 0) AS is_error_d,

        sv.serve_bucket_1_8_d,
        sn.serving_side_d,

        -- reliable per-point serve counts (no FILTER)

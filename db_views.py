...
CREATE_STMTS = {
    ...
    const AS (
      SELECT
        10.97::numeric        AS court_w,          -- doubles width
        23.77::numeric        AS court_l,          -- court length
        5.5::numeric          AS mid_x,            -- half of 10.97
        23.77::numeric / 2    AS mid_y,
        0.50::numeric         AS serve_eps_m,      -- 0.5m band
    ...
    -- D4. Serve flags (your rules; use ball coords only) ------------------------
    serve_flags_all AS (
      SELECT
    ...
    -- D5. Serve events (side from x/y, only where serve_d=true) -----------------
    ...
    -- D7. Attach each swing to latest serve at/preceding it --------------------
    swings_in_point AS (
      SELECT
        s.*,
        sp.point_number_d,
    ...
    -- D9. Bounce after each swing (passthrough) --------------------------------
    swing_bounce AS (
      SELECT
    ...
    SELECT
      ...
      -- keep names for continuity; these are ball refs now
      ...
      -- serve flag exactly per your rule
      (sfall.is_fh_overhead AND COALESCE(sfall.inside_serve_band, FALSE)) AS serve_d,

      -- error = ball speed is zero
      (COALESCE(sn.ball_speed, 0) = 0) AS is_error_d,

      sv.serve_bucket_1_8_d,
      sn.serving_side_d,

      -- counts based on serves in the point
      SUM(CASE WHEN (sfall.is_fh_overhead AND COALESCE(sfall.inside_serve_band, FALSE))
               THEN 1 ELSE 0 END

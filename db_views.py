...
-- base swings with an ordering timestamp
swings AS (
  SELECT
    v.*,
    COALESCE(
...
-- orientation (prefer column, else y-sign)
...
-- serve candidates -> dynamic centerline
serve_candidates AS (
  SELECT
...
-- normalized bounces with a preferred timestamp
bounces_norm AS (
  SELECT
...
-- attach each swing to most recent serve boundary
swings_in_point AS (
  SELECT
    s.*,
    sp.point_number_d,
...
-- per-point indices + prev/next hits
swings_numbered AS (
  SELECT
    sps.*,
...
-- first rally shot in each point (earliest non-serve)

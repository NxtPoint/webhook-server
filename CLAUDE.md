...
- **Above `TRIM_SEEK_INPUTS_PER_PASS` (12) segments it runs multiple passes** joined by the concat demuxer (`-c copy`, no re-encode) — each open input costs a socket + demuxer index and the worker is a **512 MB starter** instance, so a 100-input single pass risks OOM. Multi-pass forces CFR (`-r` + `-fps_mode cfr`) so the stream-copy join can't drift on VFR phone footage. `TRIM_SEEK_INPUTS_PER_PASS=0` = one pass regardless.

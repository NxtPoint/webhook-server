...
**AUTH CUTOVER — LIVE 2026-06-16 (de-Wix auth):** marketing "Log in / Start Free" CTAs now point at **`/login`** (our Clerk sign-in, served by the locker-room service), NOT the Wix portal. New/returning users authenticate via **Clerk** (Google/email) → standalone Render `/portal` (no Wix iframe). The Wix login + `postMessage` handoff remain intact as a fallback (the wixstudio URL still works) but are no longer linked. Server side: `auth_v2/` verifies the Clerk JWT and `client_api._guard()` accepts it **alongside** the legacy `CLIENT_API_KEY` (still the active fallback — Phase 4 hardening removes it later). Frontend: every logged-in SPA uses the shared `TFAuth` helper (`frontend/auth_client.js`, served at `/auth_client.js`) — **Clerk loads once in the portal (top frame); child iframes relay a fresh token per request via `postMessage`**. Gated by `AUTH_V2_ENABLED=1` (both services) + `CLERK_PUBLISHABLE_KEY`/`AUTH_ISSUER`/`AUTH_JWKS_URL`. Currently a Clerk **dev** instance (`definite-terrapin-9`, `pk_test_…`); swap to a Clerk **production** instance (`pk_live_…` + own Google OAuth creds) before real launch. Full plan: `AUTH-MIGRATION-PLAN.md`; status: `marketing_crm/STATUS.md`.

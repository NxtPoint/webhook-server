...
| `ml_pipeline/bounce_detector/` | CNN bounce bronze model (ADR-01 v2 — gravity-residual candidates → CNN scorer) → `ml_analysis.ball_bounces`. Runs Batch-side post-pipeline from `__main__.py` (match flow only); weights `models/bounce_detector_v2_7match.pt`. Trips rule #8. |

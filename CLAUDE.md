...
> **★ STATUS 2026-07-27: the encode now runs as a per-use AWS Batch (Fargate) job — `video_pipeline/fargate_trim/`, `TRIM_BACKEND=batch` (default). The `nextpoint-video-worker` Render service is redundant and should be SUSPENDED ($25/month saved).** Proven on `df594aea`: **7.9 min, 2.40× realtime, ~$0.135**, output 1136.3s vs 1135.8s requested. The same match on the Render worker ran **121 min and never finished** — its 0.5 CPU manages only 0.17–0.23× realtime, so ~24 min of reel needed ~2 h. Fargate bills per CPU-second, so 16 vCPU for 8 min costs about what 0.5 vCPU costs for 4 h, and nothing between trims. Read `video_pipeline/fargate_trim/README.md` before touching the trim. **`ffmpeg_trim_worker.py` is baked into the ECR image — changing it needs a Docker rebuild + push; a Render deploy alone will NOT update it.** Rollback: `TRIM_BACKEND=http` (+ resume the worker); kill switch `VIDEO_TRIM_ENABLED=0`. The sections below describe the shared encode logic, which both backends use.
...
2. *Stream the source + ONE `trim`-filter graph over a single input* → fixed `/tmp`, but a single-input `trim` graph must **decode the whole source** to reach later segments → df594aea (74 min, **8.0 GB**) hit `Command timed out after 3600s`.

Now each kept segment is **its own input** with `-ss` before `-i`, so ffmpeg seeks (HTTP range request) instead of decoding forward; only the ~30% kept is decoded. Measured 3.4× less decode work at a 30% keep ratio — the theoretical ceiling. `-ss` is keyframe-based but ffmpeg's default accurate_seek decodes forward to the exact timestamp, so cuts stay frame-accurate.

- **Source strategy is size-adaptive**: download-once + local seeks below `TRIM_LOCAL_COPY_MAX_MB` (1500), else stream via presigned URL. **Downloading is NOT the default** — real match sources reach 8 GB and cannot fit a 2 GB `/tmp`. Rollback `TRIM_STREAM_INPUT=0` forces download.
- **Above `TRIM_SEEK_INPUTS_PER_PASS` (4) segments it runs multiple passes** joined by the concat demuxer (`-c copy`, no re-encode). ffmpeg opens every input up front and queues packets for inputs the concat filter hasn't consumed yet, so RAM grows with `input_count × bitrate`. **Measured on 0.5 CPU / 512 MB against 1080p 15 Mbps: 8 concurrent inputs get SIGKILLed (OOM), 6 survives, throughput is flat from 1–6** — so a small N costs nothing and buys the headroom. Don't raise it without re-measuring at the real bitrate. Multi-pass forces CFR (`-r` + `-fps_mode cfr`) so the stream-copy join can't drift on VFR phone footage. `=0` → one pass regardless.
- `TRIM_ENCODE_TIMEOUT_S` is a **whole-trim budget shared by all passes**, not per-pass.

> **⚠ Encode is now the binding constraint, not decode (open, 2026-07-26).** With decode fixed, re-encoding the *kept* footage is what costs: **measured 0.17–0.23× realtime for 1080p CRF28 veryfast on a 0.5-CPU box**, so df594aea's 23.8 min of highlights needs **~104–141 min** — still over the 60-min budget. **A 0.5 CPU instance cannot re-encode ~24 min of 1080p within the hour, at any preset** (720p ultrafast ≈ 49 min is the only sub-budget combination measured, and it inflates the file ~5×). The structural fix is **more CPU** (upgrade the video-worker plan), not more compression. Levers, all no-redeploy: `TRIM_MAX_HEIGHT` (0 = source; 720 downscales), `VIDEO_PRESET`, `VIDEO_CRF`. Don't just raise `TRIM_ENCODE_TIMEOUT_S` — an hour-plus trim is the thing being fixed.
...
- **The worker refuses duplicate concurrent trims per task** (`video_worker_app._acquire_trim_lock`, PID-checked lock under `TRIM_LOCK_DIR`). Only the worker knows whether an encode is alive; the sweep can't see inside it, so without this a re-fire stacked a second ffmpeg over the same multi-GB source until the instance OOMed — which then *looked* like the "worker killed mid-encode" the sweep exists to fix. A lock naming a dead PID is reclaimed, so a genuinely crashed trim still recovers.
- **`TRIM_STALE_AFTER_S` must exceed the longest legitimate trim** (default raised 1800→**7200**). Measured 2026-07-26: df594aea ran healthily for 32 min and was killed by the 30-min rule, not by ffmpeg — and the give-up error said "worker likely killed mid-encode", which was simply untrue. A slow trim and a dead trim are indistinguishable from the main API's side.
- Recover orphans via `/ops/sweep-stale-trims` / `/ops/retrim`. Checks: `.venv/Scripts/python -m video_pipeline.tests.{test_trim_cmd,test_trim_lock}` (arg math + in-flight lock, instant) and `video_pipeline/tests/e2e_trim_docker.py` (real ffmpeg over a synthetic clip in Docker — the dev box has no ffmpeg and cannot test against match video).

> **Footgun:** `https://video-worker.onrender.com` — the obvious public URL — is squatted by a **foreign Node/Express app** (`"Cannot GET /trim"`), NOT our Flask worker. Never external-health-check it; use the video-worker service's Render dashboard/Logs. `VIDEO_WORKER_BASE_URL` is therefore **`sync: false` in `render.yaml` (dashboard-set, deliberately not committed)** so a blueprint sync can't clobber the real URL with the squatted one.

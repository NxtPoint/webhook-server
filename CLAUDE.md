...
- Recover orphans via `/ops/sweep-stale-trims` / `/ops/retrim`. Checks: `.venv/Scripts/python -m video_pipeline.tests.test_trim_cmd` (arg math, instant) and `video_pipeline/tests/e2e_trim_docker.py` (real ffmpeg over a synthetic clip in Docker — the dev box has no ffmpeg and cannot test against match video).

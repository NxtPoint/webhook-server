...
- **Module-level orientation** → **`<module>/README.md` first — most packages have one** (21 as of 2026-08; don't trust an enumeration here, it drifts — `ls */README.md */*/README.md`). The non-obvious ones: `video_pipeline/fargate_trim/` (the trim runbook), `docs/business/` (the non-T5 master index), `devenv/` (local Postgres), `ingest_quality/` (post-analysis sanity gate), `silver_analytics/`, `core_db/`, `marketing_crm/`.
...
- **SportAI pipeline audit + closeout status** → `docs/_investigation/pipeline_end_to_end_audit_2026-07-19.md` — the deep audit (JSON→bronze→silver→gold→dashboard) with a **★ CLOSEOUT STATUS table at the top**: the silver-derivation P1s are closed; open items are in billing/frontend/gold (deuce/ad midline, hollow-ingest billing, NULL-as-0% rendering, serve-strategy double-count, soft-delete on `vw_player`). Live state + next steps → `.claude/next_session_pickup.md`. **Validate any silver-derivation change with `python -m ml_pipeline.diag.recon_bench`** — the locked both-match gate (`c8b77210` 18/18 AND no `df594aea` regression) that replaces the old hand-rolled 18/18 check; build in `devenv/` (disposable local Postgres seeded from a read-only prod replica; `devenv/README.md`) and score it with `--db devenv`, plus `bench` green. Never ship derived-logic on a hunch — several were refuted by measurement this sprint.
- **An "empty dashboard" / "the analysis is wrong" report, or an ingest-worker OOM** → `docs/_investigation/empty_analysis_and_ingest_memory_2026-08-08.md`. **An empty match is an all-swings-invalid match**: ball tracking collapses → SportAI flags every swing `valid:false` → `_resolve_two_players` counts only `valid IS TRUE` → raises `Cannot resolve 2 players (found 0)` → zero silver rows. **That error does NOT mean SportAI failed to identify the players** (one such match had exactly 2 correct players and 228 swings, all invalid). Measured on all 10 archived payloads, **valid swings == silver rows exactly**, so the payload predicts the outcome before bronze. Codec is a *weak* signal only — 2 of 4 HEVC matches analysed fine; **don't gate on it** (an earlier version of this doc claimed 0/4 and was wrong — it used `n_rallies`, which doesn't track usability). Same doc carries the ingest memory profile (a *degenerate* payload compresses BETTER and costs MORE RAM — never infer memory from `aws s3 ls`) and the `ingest_quality/` gate.
...
13. **Don't expect a `git push` to ship a change to `ffmpeg_trim_worker.py`.** It is baked into the **ECR image** the Fargate trim job runs (`video_pipeline/fargate_trim/Dockerfile`), so it needs a Docker rebuild + ECR push — a Render deploy alone updates nothing. Same class of trap as rule #8. Full command in `video_pipeline/fargate_trim/README.md`.
14. **Don't push to `main` while work is running on a Render service whose result you care about.** A push redeploys every *active* service, and a redeploy kills whatever that instance is mid-way through — this silently destroyed two long trim runs on 2026-07-26. Currently dormant for trims (the Fargate job runs in AWS, and the video-worker is suspended so it doesn't redeploy), but it applies the moment you resume that worker, and to any long in-process job on the main API or ingest worker.
...
| ~~**Video trim worker**~~ **SUSPENDED 2026-07-27** | Docker (`Dockerfile.worker`) | `video_pipeline/video_worker_wsgi.py` |
| **Locker Room** (static) | `gunicorn locker_room_app:app` | `locker_room_app.py` |

**Only three of these are live.** The video-trim worker is **suspended** — the encode moved to a per-use AWS Batch (Fargate) job on 2026-07-27 (see Video trim pipeline). It still exists in `render.yaml` and in code as the `TRIM_BACKEND=http` rollback; don't delete it, and don't assume it's running.
...
**Deploy:** Render auto-deploys every active service on push to `origin/main` (three of the four — the video-worker is suspended). Always `git push` *before* asking the user to rerun an ingest — otherwise the shell runs stale code (rule #7). **A `git push` does NOT update the Fargate trim image or the ML Batch image** — those need a Docker rebuild + ECR push (rules #8 and #13).

### Testing & CI

No automated test suite, no linter. Functional testing is manual against the live Render DB. Do not run `pytest`.

Runnable checks that exist and are worth knowing before re-deriving them:
...
- `.venv/Scripts/python -m video_pipeline.tests.test_trim_cmd` / `.test_trim_lock` — trim arg+segment math and the in-flight lock. Instant, no deps, no DB. Run after touching `ffmpeg_trim_worker.py` or `video_worker_app.py`.
- `.venv/Scripts/python -m ml_pipeline.diag.recon_bench` — **the silver-correctness gate.** Scores `silver.point_detail` (point boundaries + point winners) against Tomo's hand-verified video GT on BOTH reference matches, locked in `recon_baseline.json`. Iron rule: a silver change must keep `c8b77210` at **18/18** AND not regress `df594aea` (both-match anti-overfit). `--db prod` (default, `tf_readonly`) reads the shipped silver; `--db devenv` scores a local rebuild; `--diff` shows per-point winner diffs. Not in CI. **Run it around any silver-derivation change** — it is the automated form of the manual 18/18 check.
- `.venv/Scripts/python -m ingest_quality.tests.test_gate` — post-analysis sanity gate (rejects empty SportAI analyses). Pure logic, instant, no deps. Run after touching `ingest_quality/`.
- `video_pipeline/tests/test_timeline_aces.py` (needs pandas) and `video_pipeline/tests/e2e_trim_docker.py` (needs ffmpeg) — **both self-skip on this Windows dev box** (numpy is blocked by Application Control; there is no local ffmpeg). Run them in a container; the docker one-liner is in each docstring. The e2e one drives a real ffmpeg over a synthetic clip, which is the only way to test the encode without match video.
...
- `silver.match_player_summary` / `silver.player_movement_grid` / `silver.match_quality` — three **non-point-detail** grains (per-player fitness/shot-mix/movement, a pre-aggregated court-occupancy heatmap grid, and SportAI's own confidence → reliability tier). Built by the **`silver_analytics/`** package from bronze tables `build_silver_v2` never reads (`player`, `player_position`, `session_confidences`, `team_session`). They **never touch `silver.point_detail`** (the 18/18 layer), reuse `build_silver_v2._resolve_two_players` for identity parity, aggregate in SQL (rule #2), and are idempotent (DELETE-then-INSERT by `(task_id, model)`). Wired best-effort into `ingest_worker_app._do_ingest` STEP 3b — a failure logs and skips, never fails the ingest.
...
- **SportAI** (`tennis_singles`): async submit → poll → delegate to ingest worker → raw archive → **sanity gate** → bronze → silver → trim → SES. The gate (`ingest_quality/`) rejects an analysis with no point structure *before* bronze, billing or the customer email — a failed SportAI run returns HTTP 200 with an empty payload.
...
> **★ STATUS 2026-07-27 — SOLVED. The encode runs as a per-use AWS Batch (Fargate) job (`video_pipeline/fargate_trim/`, `TRIM_BACKEND=batch` = default). The `nextpoint-video-worker` Render service is SUSPENDED and is no longer part of the live path.** Prod result on `df594aea` (74 min, 8.0 GB source): **8.6 min wall, 2.19x realtime, ~$0.14**, `completed`, 18.9-min reel, 55 min of dead time cut. The same match on the Render worker ran **121 min and never finished** (0.5 CPU = 0.17-0.23x realtime). Encoding is bursty, so per-second billing fits it and a fixed monthly instance does not: **16 vCPU for 8 min costs about what 0.5 vCPU costs for 4 h**, and nothing between trims. **Read `video_pipeline/fargate_trim/README.md` before touching the trim** — it is the runbook (AWS resource names, env matrix, image rebuild, diagnostics).

**Flow:** ingest worker (match) / `_do_ingest_t5` (practice) / `/ops/retrim` call `trigger_video_trim(task_id)` -> loads silver, builds the EDL -> `fargate_trim.submit` writes the work order (incl. EDL) to `s3://<bucket>/trim-jobs/<task_id>.json` and submits a Batch job -> the Fargate container runs `run_ffmpeg_trim` -> uploads `trimmed/{task_id}/review.mp4` -> POSTs the completion callback -> `bronze.submission_context.trim_status`. Marked `accepted` only after the job is genuinely submitted.

**Backends.** `TRIM_BACKEND=batch` (Fargate, live) or `http` (the legacy always-on Render worker, rollback — resume the service first). Both call the *same* `ffmpeg_trim_worker.run_ffmpeg_trim`, so the encode logic below applies to both. `VIDEO_TRIM_ENABLED=0` disables trims entirely and leaves `trim_status` NULL (the SPAs then fall back to the original video; `'failed'` would raise ops-alert noise). Ingest never fails on a trim problem — all three call sites wrap the trigger.

#### The encode (`ffmpeg_trim_worker.py`, shared by both backends)

**One `-ss/-t` seek input per kept segment + `concat`. The load-bearing property is that runtime scales with the HIGHLIGHT length, not the match length.** Two earlier designs failed for opposite reasons — don't "simplify" back to either:
1. *Download + N per-segment files + output, all in `/tmp`* -> blew Render's **2 GB `/tmp`** on long matches -> instance SIGKILLed -> trim orphaned at `accepted`.
2. *Stream the source + ONE `trim`-filter graph over a single input* -> fixed `/tmp`, but a single-input `trim` graph must **decode the whole source** to reach later segments -> df594aea hit `Command timed out after 3600s`.

Each kept segment is now **its own input** with `-ss` before `-i`, so ffmpeg seeks (HTTP range request) instead of decoding forward — only the kept ~30% is decoded (**measured 3.4x less decode work**, the theoretical ceiling at that keep ratio). `-ss` is keyframe-based but ffmpeg's default accurate_seek decodes forward to the exact timestamp, so cuts stay frame-accurate.

- **Source strategy is size-adaptive**: download-once + local seeks below `TRIM_LOCAL_COPY_MAX_MB`, else stream via presigned URL. **Downloading is NOT the default** — real match sources reach 8 GB. `TRIM_STREAM_INPUT=0` forces download.
- **`TRIM_SEEK_INPUTS_PER_PASS` bounds concurrent inputs**; above it the trim runs several passes joined by the concat demuxer (`-c copy`, no re-encode). ffmpeg opens every input up front and queues packets for inputs the concat filter hasn't consumed, so RAM grows with `input_count x bitrate`. **Measured against 1080p 15 Mbps on 0.5 CPU / 512 MB: 8 concurrent inputs = SIGKILL (OOM), 6 survives, throughput flat 1-6** — hence **4** on the cramped Render worker. The Fargate job def uses **24** because it has 32 GB. Don't raise either without re-measuring at the real bitrate. Multi-pass forces CFR (`-r` + `-fps_mode cfr`) so the stream-copy join can't drift on VFR phone footage. `=0` -> one pass regardless.
- `TRIM_ENCODE_TIMEOUT_S` is a **whole-trim budget shared by all passes**, checked before each pass. Do not reintroduce a per-pass floor — a `max(60, ...)` floor once made it bound nothing and a trim ran 121 min instead of failing fast. On exhaustion the error carries the measurement (passes done, achieved rate, projected total), and every pass logs its running rate + projection, so a too-slow box is obvious within a pass or two.
- The presigned URL must be **SigV4 in the bucket's real region** (`nextpoint-prod-uploads` is **eu-north-1**; SigV2/us-east-1 -> 400) — auto-detected via the `x-amz-bucket-region` header.
- `TRIM_MAX_HEIGHT` (0 = source resolution, never upscales) trades reel resolution for wall-clock. Scaling costs CPU itself, so the win is smaller than the pixel ratio implies.

#### EDL construction (`build_video_timeline.py` + `video_trim_api._load_silver_for_timeline`)

Points come from the silver spine; each is padded ±2 s, overlaps merge, and merged segments under `MIN_SEGMENT_S` are dropped.

- **Single-shot points are padded so ACES survive.** A point's span is `max(ball_hit_s) - min(ball_hit_s)`, and the builder drops anything under `MIN_POINT_DURATION_S` (0.5 s). An ace has exactly ONE ball-hit — the serve — so its span is 0.00 s and it was **silently cut from every reel ever produced** (measured on df594aea: 9 of 100 points dropped, all serve-only — 7 aces + 2 serve errors). `pad_single_shot_points()` gives such points a 1 s synthetic span (`TRIM_SINGLE_SHOT_PAD_S`), matching what the practice loader always did. Guarded by `video_pipeline/tests/test_timeline_aces.py`, which reproduces the bug before proving the fix.

#### Orphan recovery — two independent killers, both must stay generous

- **`TRIM_STALE_AFTER_S` (main API sweep) and `TRIM_STALE_S` (`cron_capacity_sweep.py`) must BOTH exceed the longest legitimate trim** (defaults raised 1800 -> **7200**). Neither can see inside the worker, so a slow trim and a dead one look identical. At 1800 s df594aea ran healthily for 32 min and was killed by bookkeeping, with a give-up error asserting "worker likely killed mid-encode" that was simply untrue. Give-up messages now state only what is known.
- **The HTTP worker refuses duplicate concurrent trims per task** (`video_worker_app._acquire_trim_lock`, PID-checked lock under `TRIM_LOCK_DIR`). Without it a sweep re-fire stacked a second ffmpeg over the same multi-GB source until the instance OOMed. A lock naming a dead PID is reclaimed so a crashed trim still recovers. **Batch needs no equivalent** — a job reports its own terminal state.
- `/ops/sweep-stale-trims` and `/ops/retrim` recover orphans. `/ops/retrim` does not reset `trim_attempts`, so a manually-retried task stays at its attempt count.
- **Checks:** `.venv/Scripts/python -m video_pipeline.tests.{test_trim_cmd,test_trim_lock}` (arg math + lock, instant, no deps). `video_pipeline/tests/test_timeline_aces.py` (needs pandas) and `video_pipeline/tests/e2e_trim_docker.py` (needs ffmpeg) **self-skip on the Windows dev box** — run them in a container; the docker one-liner is in each docstring.

> **Footgun:** `https://video-worker.onrender.com` — the obvious public URL — is squatted by a **foreign Node/Express app** (`"Cannot GET /trim"`), NOT our Flask worker. Never external-health-check it. `VIDEO_WORKER_BASE_URL` is `sync: false` in `render.yaml` (dashboard-set, deliberately not committed) so a blueprint sync can't clobber the real URL with the squatted one.
...
- **Video trim**: `VIDEO_TRIM_CALLBACK_OPS_KEY` (callback auth, must match main API `OPS_KEY`) — used by both backends. `VIDEO_WORKER_OPS_KEY` authenticates the legacy Render worker only (`TRIM_BACKEND=http`); the Fargate job authenticates via IAM, not a shared key.
...
- `POST /ops/retrim {"task_id", "force"}` — reset a stuck/failed `trim_status` and re-fire one trim (clean one-call re-fire; `force` re-trims a completed one).
...
`video_pipeline/` sub-packages: **`fargate_trim/`** (the live trim backend — Batch submit + container runner + its own Dockerfile; **its README is the trim runbook**) and **`tests/`** (`test_trim_cmd`, `test_trim_lock`, `test_timeline_aces`, `e2e_trim_docker` — `python -m` scripts, not pytest).
...
- `raw_archive/` — ingest-side helper (wired into `ingest_worker_app._do_ingest`): archives every SportAI payload to `s3://<bucket>/raw-json/<task>.json.gz` and raises a **SCHEMA DRIFT** alarm (log + ops email) on any new top-level key. `RAW_ARCHIVE_ENABLED` (default on). The source-of-truth keeper — SportAI's re-fetch URL expires in 1 hour, so this is the only durable copy. **Not ignorable** — it is also the diagnostic corpus every codec/quality finding was measured on.
- `ingest_quality/` — ingest-side helper, STEP 1c of `_do_ingest`, immediately after `raw_archive`: the post-analysis sanity gate. Rejects an unanalysable SportAI payload before bronze/silver/trim/billing/customer-notify. `ingest_quality/README.md` (incl. the three rejected gate designs — don't reintroduce them); check with `python -m ingest_quality.tests.test_gate`.

**SportAI-ingest + silver env flags (2026-07):** `VIDEO_QUALITY_CHECK_ENABLED` (pre-analysis `/api/videos/check` gate, default on), `RAW_ARCHIVE_ENABLED` (raw-JSON archive + drift alarm, default on), `BOUNCE_CANDIDATES_ENABLED` (recover extra floor bounces from `debug_data.ball_bounces`; **code default ON** since 2026-07-23 — the `render.yaml` value never reached the worker, so it was a silent no-op), `SILVER_RALLY_CONTIGUITY` (**default ON** — rally ends at the first >5s break, not the legacy re-anchor; makes point winners reproducible), `SILVER_RALLY_IIR_MIN_COVERAGE` (=1.01 → the `is_in_rally` gap-break escape is OFF), `SILVER_SERVE_SOURCE` (`geometric` default / `sa` / `auto` — SA/auto path video-validated but not enabled), **`SILVER_SERVE_GAP_ANCHOR`** (**default ON**, `SILVER_SERVE_GAP_ANCHOR_S`=30 — a new point also anchors when consecutive serves are >30s apart; fixed all 6 point-merges on `df594aea`, exact-1:1 86→98, and held `c8b77210` at 18/18. The merges were a **silver anchor gap, not a bronze missed-serve ceiling** — a missing serve breaks deuce/ad alternation and glues same-side points). All have env rollbacks. Full context: the filter-contract doc + `.claude/next_session_pickup.md`.

**Post-analysis sanity gate (`ingest_quality/`, 2026-08-08):** `INGEST_SANITY_GATE_ENABLED` (default 1; `0` = assess-and-log only), `INGEST_SANITY_MIN_FINAL_CONF` (0.55, warn-only), `INGEST_SANITY_SUSPECT_CODECS` (`hevc,h265`, warn-only). A failed SportAI analysis returns **HTTP 200 with a well-formed but empty payload** — before this gate the ingest "succeeded", silver wrote nothing, and the match still showed `completed` with 0 points in the customer's sidebar (two real matches, 2026-08-08). Rejects on **0 valid swings** (the direct cause) or **0 rallies AND 0 bounces**; validated against all 10 archived payloads — rejects iff the match produced zero silver rows. Runs as STEP 1c *after* the raw archive so a rejected payload is kept. **Don't re-key it on `ball_positions`, rallies-per-minute, or codec** — all three were measured and all three give false results (`ingest_quality/README.md`).

**Footgun — `last_status='completed'` outlives a later failure.** Bronze (STEP 2) sets it; nothing resets it if silver/trim then fails. So a broken match reads `completed` to `gold.vw_client_match_summary` (the customer sidebar) **and to `billing_import_from_bronze`, which selects exactly `last_status='completed'` with no consumption record** — i.e. running the bulk usage sync would bill it. Still open; the sanity gate only prevents the empty-analysis cause.

**Ingest-worker memory (2026-08-08):** the worker is on Render **`standard` (2 GB)** — upgraded from `starter` (512 MB) after a payload that parsed to a **558 MB dict** OOM-killed it four times. `aws s3 ls` on `raw-json/` is **not** a memory proxy: that payload was 5.8 MB gz where a healthy one was 11.0 MB (97.7%-zero phantom heatmaps gzip 13.5× and expand 7× in RAM). If you touch the ingest path, don't reintroduce a whole-payload `json.dumps` — `raw_archive._gzip_stream` and `ingest_bronze._encode_payload`/`_sha256_payload` stream deliberately.

**Ingest resilience flags (2026-07-26):** `DEBUG_DATA_MAX_BYTES` (=2000000; caps the `bronze.debug_event` JSONB — a 7.6 MB `debug_data` blob on a badly-tracked match severed the DB connection and aborted the whole bronze txn; oversized → stored as a stub, the useful bits are promoted separately), `INGEST_DB_RETRY_MAX`/`INGEST_DB_RETRY_BASE_S` (worker retries the whole idempotent ingest on a transient DB error — Render PG failover / "in recovery"), `SWEEP_SA_MAX_ATTEMPTS`, `TRIM_STALE_AFTER_S`/`TRIM_SWEEP_MAX_ATTEMPTS` (**and `TRIM_STALE_S` in `cron_capacity_sweep.py` — a second, blunter killer of the same rows; keep the two in step**), `TRIM_STREAM_INPUT`/`TRIM_ENCODE_TIMEOUT_S`/`TRIM_PRESIGN_EXPIRY_S`/`TRIM_SEEK_INPUTS_PER_PASS`/`TRIM_LOCAL_COPY_MAX_MB`/`TRIM_MAX_HEIGHT`/`TRIM_LOCK_DIR`/`TRIM_SINGLE_SHOT_PAD_S`, `S3_BUCKET_REGION`. Trim backend selection + Fargate sizing (`TRIM_BACKEND`, `VIDEO_TRIM_ENABLED`, `TRIM_BATCH_*`) → `video_pipeline/fargate_trim/README.md`. **Silver footgun:** `build_silver_v2` runs `ALTER TABLE silver.point_detail DROP COLUMN shot_q/…` in a **SAVEPOINT** — legacy `ss_.*` + gold `SELECT *` views still bind those columns (`DependentObjectsStillExist`), and without the savepoint a blocked DROP poisons the txn and fails every silver build.

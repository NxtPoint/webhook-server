...
- **An "empty dashboard" / "the analysis is wrong" report, or an ingest-worker OOM** → `docs/_investigation/sportai_hevc_and_ingest_memory_2026-08-08.md`. **Check `meta.video_info.codec` in the archived payload FIRST: SportAI produces nothing usable from HEVC/H.265** (measured 0/4 hevc vs 6/6 h264; players track fine, only the ball detector dies — so the customer's footage looks perfect and *is* perfect). iPhones record HEVC by default. Same doc carries the ingest memory profile (a *degenerate* payload compresses BETTER and costs MORE RAM — never infer memory from `aws s3 ls`) and the `ingest_quality/` gate that now rejects empty analyses before they're billed.
...
**Post-analysis sanity gate (`ingest_quality/`, 2026-08-08):** `INGEST_SANITY_GATE_ENABLED` (default 1; `0` = assess-and-log only), `INGEST_SANITY_MIN_FINAL_CONF` (0.55, warn-only), `INGEST_SANITY_SUSPECT_CODECS` (`hevc,h265`, warn-only). A failed SportAI analysis returns **HTTP 200 with a well-formed but empty payload** — before this gate the ingest "succeeded", silver got nothing, the customer was emailed a ready dashboard and **a credit was consumed** (two real matches). Rejects only on **0 rallies AND 0 bounces**; runs as STEP 1c *after* the raw archive so a rejected payload is kept. Don't re-key it on `ball_positions` or rallies-per-minute — both were measured and both produce false results (`ingest_quality/README.md`).

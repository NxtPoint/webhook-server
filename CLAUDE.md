...
| `ml_pipeline/stroke_classifier/` | Optical-flow R(2+1)D swing-type classifier (ADR-02 v2, both players) → bronze `stroke_class`. **In the Batch image and ENABLED** (`SWING_CLASSIFIER_ENABLED` default 1, `pipeline.py:646`; rev-80 job-def `=1`) — now a 4-class model (the `other` class was added); swing bench LOCKED at macro-F1 0.7468 (GPU, `bench_swing_type`). Silver projects `stroke_class` verbatim. ⚠️ Output verified on `ea085d50` only — pending proof on the next real upload (existing tasks have NULL `stroke_class`). Rollback: `SWING_CLASSIFIER_ENABLED=0` (no rebuild). |

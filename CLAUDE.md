...
Fire-and-forget async: ingest worker (match) or `_do_ingest_t5` (practice) calls `trigger_video_trim(task_id)` → loads silver, builds EDL → POSTs to video worker → worker spawns detached subprocess → **streams source from S3 (presigned URL) → ONE `trim`+`concat` ffmpeg pass** → uploads `trimmed/{task_id}/review.mp4` → callback updates `bronze.submission_context.trim_status`.

**Streaming single-pass (2026-07-26, `ffmpeg_trim_worker.py`):** the old path downloaded the full source + wrote N per-segment re-encodes + the output all to `/tmp`, blowing Render's **2 GB `/tmp` limit** on long matches (100+ segments) → instance SIGKILLed mid-encode → trim orphaned at `accepted`. Now the source is **streamed** (nothing downloaded) and trimmed in **one filtergraph pass** (only the output touches `/tmp`); recover orphans via `/ops/sweep-stale-trims` / `/ops/retrim`. The presigned URL must be **SigV4 in the bucket's real region** (`nextpoint-prod-uploads` is **eu-north-1**; SigV2/us-east-1 → 400) — region auto-detected via the `x-amz-bucket-region` header. Rollback: `TRIM_STREAM_INPUT=0` (download fallback).

> **⚠ Trim NOT fully fixed on LONG matches (open, 2026-07-26):** the single-input `trim`-filter graph decodes the *entire* source, which is too slow over HTTP on a long match (df594aea, 74 min → `Command timed out after 3600s`). The `/tmp` kill is gone but this replaced it. **Real fix (next session):** rewrite `run_ffmpeg_trim` to use N `-ss/-to` **seek** inputs + `concat` (fast-seek each segment, decode only the ~30% kept). Short matches likely already work. See `.claude/next_session_pickup.md` §Parked.

> **Footgun:** the committed `render.yaml` `VIDEO_WORKER_BASE_URL` = `https://video-worker.onrender.com` currently resolves to a **foreign Node/Express app** (`"Cannot GET /trim"`), NOT our Flask worker. Trims work only because the live main-API env points to the real worker URL. Don't health-check `video-worker.onrender.com` externally, and fix the committed value to the real service URL.

...
**Sequencing caveat (added 2026-05-21, narrowed 2026-05-21 PM):** the **silver bench** (item #2) should be **baselined AFTER Phase 5a's Step F measurement lands**, not before — otherwise the baseline is immediately stale. Build the harness scaffolding (fixture loader, bench runner, baseline file format) in the build session, but don't lock the per-task expected row counts until 5a's bounce-extractor data has flowed through silver and the new 880dff02 row count is known. Locking earlier means re-baselining day one, which defeats the harness's purpose.

**This caveat applies ONLY to the silver bench, NOT the ball-tracker bench (item #4).** The ball-tracker bench reads raw video frames + measures recall against SportAI-side silver (`silver.point_detail WHERE model='sportai'`), neither of which Phase 5a touches. Ball-bench baseline can be locked as soon as the harness produces consistent numbers — no need to wait on Phase 5a. (If the bench design changes to include T5-side bronze ball detections in its metrics, this changes — re-read this caveat then.)

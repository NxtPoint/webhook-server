...
1. **🔴 Bounce over-generation (NEW #1 priority, dominant).** Even main-only silver is 121 vs SA 84 (+44%). **Diagnosis (b008888c vs SA, overnight):** NOT pre/post-point leakage (only 2 pre, 0 post) and NOT simple duplicates (3 gaps <0.4s). The driver is **74 of 121 T5 shots have NO SA shot within 1.0s (61% unmatched)** — i.e. low bounce PRECISION (matches the documented 27%). T5 ALSO misses ~37 SA shots (FN). So bounce is weak on BOTH precision and recall. **Key caveat:** T5 silver is **BOUNCE-driven** (1 row/bounce) while SA is **STROKE-driven** (1 row/stroke) — different event types ~0.3-0.5s apart, which inflates the apparent mismatch; some "phantom" T5 rows are 2nd-bounces / non-stroke bounces. **Fix paths:** (a) bounce precision/phantom-rejection in the main pass + silver bounce-validity (must-cross-net etc.); (b) revisit the gated **stroke-driven silver** (`T5_STROKE_DRIVEN_SILVER`) per rule #11 — it directly fixes the event-type mismatch, but only AFTER bronze stroke attribution is right; (c) roi_prod dedup/merge so roi_bounces' ~16 min adds recall without flooding. Highest-leverage reconciliation work left.

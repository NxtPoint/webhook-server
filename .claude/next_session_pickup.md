...
> - **Calibration follow-up — FULLY FIXED on `main`, validated locally, NOT yet deployed (rides the next runtime rebuild — no separate deploy; just rebuild from `main`).** Two commits closed the rev-57 prod-vs-local gap (43% vs 86%):
>   - `0ceec5b` **lock the BEST fit, not the first above the floor** (`court_detector.py`): early-exit at `COURT_GREAT_COVERAGE=0.70`, keep searching past mediocre fits, best-so-far at the cap (monotonically safe, best ≥ first).
>   - `8356237` **`fit_calibration` selects by projection quality, not pixel-rms** (`camera_calibration.py`): ranks radial/piecewise by court-grid coverage + y_span, rms only as tiebreak — stops the overfit piecewise (tiny rms, collapsed court) from being chosen over the radial.
>   - **Result:** match-4 self-heal now locks **RADIAL at 94% coverage** (was piecewise 86% → originally 43%/0%), sound geometry far `y=13.9` → near `y=24.6`. bench_calib 4/4, serve bench 20/24·23/24 green throughout.
>   - **⚠️ For the runtime agent:** these touch `court_detector.py` + `camera_calibration.py` (Batch-side). Your next rebuild from `main` carries them automatically — no action needed beyond building from `main`. Nothing else of mine is outstanding.
>   - The **fps=60 label-alignment bug is the runtime agent's backlog** (bigger M4-training lever). Fix E (wide-camera) still dormant on `main`, awaits a fisheye fixture.

# Next-session pickup — 2026-06-15 — ✅ FIRST FULL TRAIN DONE + ✅ BOUNCE+SWING DEPLOYED (detection image **eu rev 80 / us rev 61**, amd64 `19ef92be`). serve+hit HELD. main @ `788aaf9`.

> **DEPLOY 2026-06-15 (bounce + swing):** Rebuilt the detection image with the retrained **bounce** (`bounce_detector_v2_7match.pt`) + **4-class swing** (`swing_classifier_v2.pt`) weights + the 4-class `stroke_classifier` code; dual-region ECR push (cross-region digest equal `19ef92be`); job-defs **eu 79→80 / us 60→61** with `SWING_CLASSIFIER_ENABLED=1` + `BOUNCE_CNN_THRESHOLD=0.85` (all 16 prior env vars preserved). Bench green pre-build. **serve HELD** (new weights not confirmed > signed-off deployed serve — bench before deploying). **hit HELD** (not wired into prod inference; the +14pp precision win is banked, needs wiring code). Verify on the next real T5 upload: bounce precision holds + swing_type now populated (not all 'other'). Rollback: re-register job-def at the prior digest / unset the two env flags.

...
> ✅ **2026-05-29 (calibration agent, later) — DEGENERATE LOCK FIXED + VALIDATED LOCALLY (`5dc5e97` on `main`). NOT yet rebuilt/redeployed — rev 56 still runs the OLD calibration.** Reproduced the caretaker's finding exactly on the real weights/video: match 4 locked a geometry-"VALIDATED" detection on **8 clustered observations** → piecewise lens fit **overfit (rms=0.0)** → projected 0% (or, with a coverage-only check, 80% while collapsing every pixel to y≈23.77). **Root gap: nothing validated the calibration OUTPUT.**
> **Fix (`court_detector.py`):** a **positive projection self-test before lock** — `_projection_quality()` projects a court-region pixel grid and the lock now requires `coverage ≥35% AND y_span ≥6m AND x_span ≥3m` (robust p90−p10). Rejects both the 0%-coverage overfit AND the y-collapsed "80% but degenerate" case the caretaker's leads pointed at. On failure the detector **keeps sampling** (re-fit every +4 obs) for more/better-spread observations; **fail-loud at the hard cap** rather than lock garbage. `COURT_MIN_CALIB_OBS` 8→12, hard cap 18k→36k frames, fit wrapped in try/except (clustered obs can raise singular-matrix).
> **Receipts (LOCAL, real weights + match-4 original):** now self-heals at frame 5550 / 24 obs to a **real** fit (`rms=2.84px`, 86% coverage, monotonic geometry far `y=15.7` → near `y=24.4`, x≈5.1 centre) — matches the hand-fed `proof.py`. `bench_calib` 4/4, serve bench `20/24 & 23/24` green. **Honest status: validated on this dev box, NOT yet confirmed in a prod Batch run** — needs a rebuild (court_detector is Batch-side) + re-run.
> **Deploy:** the fix is on `main`, so the **next Batch rebuild carries it automatically** — recommend bundling with the SAHI rebuild so ONE build lands match 4 *correct AND fast* (calibration self-test + SAHI relax + Fix E dormant, all from `main`). A solo calibration-only rebuild would land match 4 with correct coords but still SAHI-slow (~6.5h). Memory: [[feedback_calibration_lock_window]].

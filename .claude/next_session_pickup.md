...
**Bounce CNN v2 — BATCH BRONZE STAGE BUILT + VALIDATED + COMMITTED (`68fdf12`), not yet rebuilt/consumed.** `ml_pipeline/__main__.py` runs the CNN (gravity_residual, thr 0.5, in-memory features, rally≈in_rally) post-pipeline → writes `ml_analysis.ball_bounces` (survives re-ingest). Validated end-to-end on b008888c: **197 bounces, precision 34%, recall 41%** vs SA 162 (vs velocity-reversal 343/20%/43%) — count ≈ SA, precision ~2×. torch+weights are Batch-only (Render main API has neither — confirmed), so it MUST run in Batch; the rally-from-serve_events dependency was dissolved (rally gate barely moves precision). **REMAINING for the deploy:**
- **Rebuild** (rule #8) to land the stage live (Task #11) — couples with the serve consumer.
- **Serve migration** (Task #10, the consumer / "visible win") = the BIG, CI-SENSITIVE piece: point serve_detector's bounce input at `ball_bounces` instead of `ball_detections.is_bounce`. ⚠️ **Bench-fixture entanglement:** the CI fixtures (a798eff0/880dff02) carry `is_bounce` in their pickled ball_rows, NOT `ball_bounces` — migrating serve's bounce source would need the fixtures regenerated or the path decoupled, or the bench breaks (rule #9). Plus the documented pass-3 serve-anchor rework. **This deserves a focused session, not a tail-of-session rush.**

...
**Next session's job:** **(1) Near-side stroke precision** — cut the near false positives (108→~43). This is the last thing keeping gated stroke-driven active (151) above SA (84). Likely needs the trained stroke classifier (Q1-D) or a precision gate (e.g. require ball-proximity / rally-window support for a near peak), NOT a threshold tweak. **(2) Q2-B end-anchored A/B identity.** Only flip `T5_STROKE_DRIVEN_SILVER` on once near reconciles too.
...
2. ~~Far fh/bh mirror~~ **DONE (`a8479a8`)**. ~~Far velocity size-normalisation~~ **DONE (`956b65a`)** — far stroke attribution now 165/106 (was 208/34); gated stroke-driven far active 27→43 (SA 41). **Far is fully unblocked.** Remaining gate blocker: **near-side stroke precision** (gated stroke-driven near 108 vs SA 43 — false-positive peaks) → trained classifier (Q1-D) or precision gate. Then per-hit far fh/bh refinement (also Q1-D).

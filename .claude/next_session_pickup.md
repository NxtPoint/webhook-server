...
**Near-side precision — INVESTIGATED 2026-05-25, confirmed a TRAINED-CLASSIFIER (Q1-D) job, no heuristic shipped.** Three principled precision gates were probed on M1 (the only match with SA truth) and all failed to cleanly separate the ~65 near false positives:
- **Ball-proximity at contact:** unusable — 58% of near hits have no ball detection within ±5 frames, and ball court_y (Phase 7 noise) doesn't separate near/far hits.
- **Rally-alternation collapse** (singles strokes alternate sides): over-corrects — near 108→34, far 43→26 (both under SA) because far strokes are sometimes missed *within* a rally, making real near strokes look consecutive.
- **Time-gated same-side collapse:** near plateaus at ~69 (still +26 over SA 43) even at a 3s gap, and already drags far to 33 (under SA 41). The near false positives are rally-paced (1.5-3s apart), not tight duplicates.
Conclusion: near false positives are temporally/structurally indistinguishable from real strokes; only a learned stroke-motion signature separates them. **Validating any precision gate requires per-stroke ground truth (dual-submit reconciliation) — the same data Q1-D trains on** — so a count-fitting heuristic on one match would overfit. Don't ship one.

**Next session's job:** **(1) Q1-D — train the far/near stroke classifier** on dual-submit pairs (SportAI as teacher; Phase 5c is accumulating `training_corpus`). This is now the single remaining blocker on the stroke-driven gate (far is fixed; near needs the learned signature). Weights go to `ml_pipeline/models/stroke_classifier.pt` (path already wired, currently absent). **(2) Q2-B end-anchored A/B identity.** Only flip `T5_STROKE_DRIVEN_SILVER` on after (1).
...
2. ~~Far fh/bh mirror~~ **DONE (`a8479a8`)**. ~~Far velocity size-normalisation~~ **DONE (`956b65a`)** — far stroke attribution now 165/106 (was 208/34); gated stroke-driven far active 27→43 (SA 41). **Far is fully unblocked.** Remaining gate blocker: **near-side stroke precision** (gated stroke-driven near 108 vs SA 43 — false-positive peaks). INVESTIGATED 2026-05-25: three heuristic gates (ball-proximity, alternation, time-gated collapse) all fail to separate cleanly (see exec summary) → confirmed a **trained-classifier (Q1-D)** job, NOT a heuristic. Per-hit far fh/bh refinement is also Q1-D.

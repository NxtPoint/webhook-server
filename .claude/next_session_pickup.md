...
> 🔴 **2026-05-29 OVERNIGHT (match-4 re-run caretaker) — RE-RUN FAILED at the 6h cap; calibration fix WORKS but SAHI now dominates (76% of wall). Match 4 did NOT land. Fix already prototyped — needs daylight Batch deploy.**
> Ran match 4 (`ca475740`) on rev 56 / g5 (Batch job `6000423a`) end-to-end with live monitoring.
> - ✅ **Calibration fix CONFIRMED WORKING in prod:** `court_calibration: LOCKED VALIDATED detection after 2220 frames (inliers=11, confidence=0.79). No more CNN runs.` Fix G found rally-footage keypoints exactly as designed. **NOT wide-angle** (agrees with the calibration session's live-repro finding; my earlier wide-angle hypothesis was wrong).
> - ❌ **Hit the 6h Batch cap at 74% (frame 65,900/71,915), wrote NO bronze → match 4 did not land.** Root cause from the live `player_sub @ frame 65900`: **SAHI = 16,101s of 21,119s = ~76% of total wall, skip rate 8/58,880 (~0%).** The perf stack cut `full_yolo` to 10% but SAHI ballooned → effective ~320 ms/fr, SLOWER than the 183 baseline. Ironic interaction: fixing calibration gave SAHI a real `court_bbox` to tile every detect-frame, and the skip rule never fires.
> - **Why SAHI never skips (root-caused + prototyped overnight by the opportunities agent):** SAHI skip Rule A requires the far player's feet to project to `court_y ≤ 5.0m`; a far player standing mid-court (5-9m) fails it → `has_far_pose` never true → SAHI runs every detect-frame. Prototype on branch **`opt/overnight-findings`**: env-gated **`SAHI_SKIP_A_FAR_YMAX`** (recommend `8.0`), plus `MOG2_DOWNSCALE` (motion_mask ~12%/38ms-fr) and the Lambda command fix. Report: `docs/_investigation/batch_optimisation_findings_2026-05-28.md`. **Agent could NOT run bench in its sandbox (python denied) — proved neutrality by construction; a human MUST run `python -m ml_pipeline.diag.bench` before deploy.**
> - 🐞 **CONFIRMED PROD BUG:** the job-def STORED command AND `lambda/ml_trigger.py:85` both repeat the `python -m ml_pipeline` ENTRYPOINT prefix → container dies at argparse (`unrecognized arguments: -m ml_pipeline`). The auto-spawn path (`upload_app.py:923`, args-only) is correct. My match-4 submit used an args-only `containerOverrides.command` to bypass it. **Direct S3 uploads are currently BROKEN on rev 55+** — fix the Lambda (branch) + job-def stored command in daylight.
> - **State:** g5 CE idle (`desiredvCpus=0`, $0); queue still g5→g4dn→Spot; `ml_analysis` for ca475740 = empty (cleared pre-run, no bronze written). Nothing burning money. Did NOT resubmit (would repeat the timeout).
>
> **MORNING PATH to land match 4 (all Batch-side → daylight + bench-green required):**
> 1. Review/merge branch `opt/overnight-findings` (SAHI skip relax + MOG2 downscale). **Run bench first.**
> 2. Docker rebuild → dual-region ECR → new job-def rev cloning 56 (preserve all env) + set `SAHI_SKIP_A_FAR_YMAX=8.0` (+ optionally `MOG2_DOWNSCALE=2`). BATCH-SIDE CHECKLIST.

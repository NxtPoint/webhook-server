...
| **Swing** | 4-class R(2+1)D classifier (`stroke_classifier/`, ADR-02) → `player_detections.stroke_class` | `training/train_swing_type.py` (GPU) → `swing_classifier_v2.pt` | `bench_swing_type.py` → `bench_baseline_swing_type.json` — **NOW LOCKED 2026-06-15 (macro-F1 0.7468; fh .767 / bh .753 / oh .876 / other .591, n_val 1264)**. Blessed on GPU via `batch_train --bench swing` (Batch `ten-fifty5-ml-train:3`, Tesla T4) — the torchvision bench can't run on the broken local CPU torch. | `stroke_classifier/` in Batch image; the `other` class (F1 0.59) is the weak point. |
...
5. **Swing** — bench **LOCKED 2026-06-15** (macro-F1 0.7468). Runs ONLY in the GPU training image (`batch_train --bench swing`, job-def `ten-fifty5-ml-train:3`) — local CPU torch is broken for the R(2+1)D forward. All 5 facts now have a committed gate (4 model benches + the identity rule gate) → **5/5**.

## How to re-bench swing on GPU (the only torchvision bench)
`aws batch submit-job --job-queue ten-fifty5-ml-queue --job-definition ten-fifty5-ml-train --container-overrides '{"command":["--fact","swing","--bench"],"resourceRequirements":[{"type":"VCPU","value":"4"},{"type":"MEMORY","value":"15360"},{"type":"GPU","value":"1"}]}'` → it builds the v3_4class dataset + runs the bench + prints the result JSON between `BENCH_RESULT_JSON_BEGIN/END` in CloudWatch (`/aws/batch/ten-fifty5-ml-pipeline`). Scrape it → write `bench_baseline_swing_type.json`. ~11 min (dataset build dominates). After a swing TRAINER change, rebuild the train image + register a new rev first (the image bundles `diag/` since rev 3).

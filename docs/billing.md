...
| File | Purpose |
|---|---|
| `models_billing.py` | SQLAlchemy ORM declarations (`Account`, `Member`, `EntitlementGrant`, `EntitlementConsumption`) + table relationships. **Schema bootstrap is not here** — it's split across `db_init`, `client_api._ensure_member_profile_columns`, `billing_service._ensure_technique_columns`. |

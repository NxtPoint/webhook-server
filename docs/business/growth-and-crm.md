...
| gclid → Google Ads offline conversions | `offline_conversions/` + `frontend/attribution.js` + `client_api` `/api/client/acquisition` | `GOOGLE_ADS_FEED_USER` / `GOOGLE_ADS_FEED_PASS` gate the CSV feed (**404/dark until set**) | ✅ **code LIVE 2026-07-11** — ported from CourtFlow/nextpoint as a **byte-identical shared module** (edit both in lock-step). gclid captured → `core.acquisition`; a paying-buyer event ledgers `core.offline_conversion`; `GET /feeds/google-ads/offline-conversions.csv` (HTTP Basic) serves Google's scheduled upload. 1050 has **no own Ads account yet** → nothing consumes its feed. See the section below + memory `project_offline_conversions_gclid`. |
...
## Google Ads acquisition — gclid → offline conversions (code LIVE 2026-07-11)
Ported from the CourtFlow/nextpoint repo as a **SHARED, byte-identical package** (`offline_conversions/`) — **edit BOTH repos in lock-step** (like the `analytics.js` beacon). Makes Google Ads bid for people who **pay**, not just click.
- **Capture:** `frontend/attribution.js` (auto-injected next to `analytics.js` by `locker_room_app._html`) records the first gclid/gbraid/wbraid/utm on landing → flushes on a logged-in page (`?email`+`?key`) to `POST /api/client/acquisition` (`client_api.py`, resolves the buyer by email) → `core_db/repositories/acquisition.record_acquisition` fills `core.acquisition.gclid` (**first-touch wins**; that column was previously dark).
- **Ledger + feed:** a paying-buyer event (`credit_purchased` / `subscription_started`) hits the `track()`/`_emit()` funnel's 4th forward (`offline_conversions.recorder.record_from_emit`, keyed by email → `core.app_user` → `core.acquisition.gclid`) → writes a `core.offline_conversion` row; `GET /feeds/google-ads/offline-conversions.csv` (HTTP Basic auth `GOOGLE_ADS_FEED_USER`/`PASS`, **404/dark until set**) serves Google Ads' scheduled upload. **No developer token** — the Ads API Center is manager-accounts-only, which is *why* the CSV route.
- **Activation:** set `GOOGLE_ADS_FEED_USER`/`PASS` on the main API service. But 1050 has **no Google Ads account of its own yet**, so nothing consumes its feed until it advertises separately (the NextPoint Tennis Centre account consumes the *nextpoint* feed). The Ads conversion action must stay named exactly `Offline purchase` (matches `recorder.CONVERSION_MAP` — the only per-repo glue). Boot init + `register()` in `upload_app.py`. Account-side runbook (the nextpoint truth): `C:\dev\nextpoint\docs\specs\GOOGLE-ADS-PLAN.md`.

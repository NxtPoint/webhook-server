...
- **✅ Gravity-residual signal VALIDATED to have the recall headroom (b008888c, `_candidate_frames_from_gravity_residual`):** at thr=10px it emits 761 candidates with **recall 70% (113/162)** vs the velocity-reversal's 43% ceiling — i.e. the real bounces ARE recoverable from this signal, which the image-y reversal fundamentally misses. Precision is 15% at thr=10 (by design — it's a recall-oriented candidate generator; that's the **CNN's** job to filter). Threshold sweep: thr=10 → 70%rec/15%prec (n=761); thr=20 → 52%/17% (n=481); thr=30 → 39%/17% (n=355). **Conclusion: the gravity_residual+CNN-v2 path has the headroom to beat today's 20%/43% on BOTH axes — IF the CNN filters the 761 candidates to ~the 113 real ones.** Next-session validation (NOT done here — needs the bounce harness to assemble wrists/rally/court features correctly): run `detect_bounces_offline(weights_path=bounce_detector_v2_7match.pt)` on b008888c, measure post-CNN precision/recall vs SA. If post-CNN precision clears ~40%+ at recall ≥50%, deploy `BOUNCE_CANDIDATE_MODE=gravity_residual` + v2 weights (Batch rebuild).

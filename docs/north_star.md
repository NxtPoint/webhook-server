...
| [ADR-02](./_investigation/adr_02_swing_type_classifier_plan.md) | Swing-type classifier — R(2+1)D-18 on 16-frame optical-flow ROI | **CORPUS EXTRACTOR LIVE 2026-05-28** (`ml_pipeline/training/label_swing_types.py`, dual-kind `_label_pair_now`). 3 backfilled rows (775 labels) + Corpus 4 (~397 more) auto-lands via hook. Model still untrained — need ~2-3k labels (~5-10 more matches). | Independent of bounce |

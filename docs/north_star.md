...
**2026-06-16 — PROVING RUN: serve over-emission is a SHIPPABLE DEV FIX (not training); bronze NOT yet build-done but the list is short.** Reference video re-run clean on rev-80 (SA `079d2c62` ↔ T5 `375198f5`) — the first-ever clean current-code run on the canonical test match. Far-pose A/B (offline, `model_far` ACTIVE — the bench fixtures can't show this, they carry no model candidates): **retiring the far-POSE serve path drops serves 55→30 (vs SA 24) with ZERO recall loss (18/24 both ways) and precision 33%→60%** — the 25 far-pose events were pure FPs the trained model already covers. Shipped behind env A/B switch `SERVE_FAR_POSE_ENABLED` (`bf99758`, default ON = bench-neutral; set `=0` to retire). Volley ~26× over + bounce 28-vs-162 = genuinely TRAIN (bounce recall). Swing classifier PROVEN on a real upload (257 `stroke_class` rows — the "unproven" caveat is dead). Point/game 34/6 self-heals off the serve fix (silver untouched). **The earlier "BRONZE BUILD COMPLETE / train next" banner was WRONG: training cannot remove the far-pose FPs (separate unioned path) — the fix is architectural (retire far-pose) + bounce training.** Receipts: `.claude/audit_bronze_build_2026-06-16.md`. New **RULE 6** below codifies reconciliation + exclusion (agreed with Tomo 2026-06-16).

**2026-06-14 PM — SWING (2nd-last model) 4-CLASS BUILD DONE + SEAMLESS GPU TRAINING ENV BUILT + ALL 5 TRAINING BENCHES LOCKED.** Three parallel workstreams landed (main @ `89d63ac`). **(1) SWING build:** ADR-02 revised to **4 classes `{fh, bh, overhead, other}`** (live data: SA already labels `other` 291× — was discarded; it lets the classifier REJECT the hit detector's false positives, the cause of the 3-class model losing the per-hit gate 32% vs 38%). **Volley split out as its own boolean fact** (SA `player_swing.volley`, 96 pos, cross-cuts swing types — derive+validate, NOT a class). Silver swing heuristics **DELETED** (`_infer_swing_type_*`, ~171 lines) — projects `stroke_class` verbatim; T5 silver not prod-consumed so safe. 4-class dataset rebuilt (`swing_type_v3_4class`, 1757 hits incl. 169 `other`). **(2) GPU TRAINING ENV:** the start/stop CPU struggle is fixed — training now runs as **AWS Batch GPU one-off jobs** in an image FROM the detection image (inherits working CUDA torch 2.3.1+cu121; the local CPU `torch==2.11.0` is BROKEN for torchvision — `NP_SUPPORTED_MODULES`). One command per fact: `submit_train_job.py --fact <serve|hit|bounce|swing>`. Runbook: `.claude/training_environment.md`. **(3) BENCHES:** all 5 facts now have an enforceable committed gate — bounce (rec 18%/prec 23%) + identity (100%) LOCKED this session; serve (CI) + hit already locked; bench_bounce/hit/identity now exit-1-on-regression (were print-only). Map: `.claude/training_harness_status.md`. **NEXT: one-time training-image build+push+jobdef register (needs Docker running) → cheap GPU smoke (`--fact bounce`) → run swing 4-class train → then the train-all-5 accuracy push** (still gated on Tomo's full-res uploads for sharp-far, DoD #7/#8). Both AUTO_ corpus flags confirmed ON (corpus accrues; 6/4 stall = no uploads since). See `.claude/next_session_pickup.md`.
...
> recall (566 vs SA 20); all verified on `ea085d50` (re-verify on the next real upload).
> **5/5 training benches LOCKED (incl. swing 0.7468, GPU).** Before training: finish the
> **hardening/cleanup sprint** (`docs/_investigation/t5_cleanup_inventory.md`, Phase 1 done).
> Field-by-field: `docs/_investigation/bronze_silver_18_audit.md`.
...
**★ DEFINITION OF DONE — HIT + BOUNCE (the gate before SWING). Verified live 2026-06-14: BUILD + ARCHITECTURE fully DONE; only TRAIN-LAST remains.** The far-side accuracy gap on BOTH facts is one root cause (far ball/player too coarse), proven un-movable by heuristics → training-gated. **DONE:** bounce model emits (`ball_bounces`) ✅; hit model emits (`stroke_events`) ✅; bounce precision lever (thr 0.70, rev 79/60) ✅; architecture decided hit-driven ✅; **`bench_hit` locked ✅ (#6 — baseline NEAR gate 67% / FAR gate 19% / prec 54%)**; **silver FLIPPED hit-driven ✅ (#9 — `T5_STROKE_DRIVEN_SILVER` default ON 2026-06-14; Tomo flipped early as T5 silver isn't prod-consumed; validated 78c32f53 pass1 110→174 rows clean; rule #11 updated)**. **REMAINING (all TRAIN-LAST / data):** (5) verify bounce output survives re-ingest — 0 rows on all existing tasks, claim unverified [verify next upload]; (7) **sharp-far footage accrues — new FULL-RES uploads through rev-79** [DATA, Tomo]; (8) retrain bounce CNN + hit model on sharp-far → far reaches ~70-80% bar, measure via `bench_hit`/`bench_bounce` [TRAIN, gated on 7]. THEN → swing (2nd-last model). **Forward progress now = #7 (Tomo uploads full-res) → #8 retrain. The whole build/architecture is in place; the ball is in the data court.** Residuals (correct-by-design holds, NOT debt): hit-driven path still reads bounce coords from `is_bounce` (bounce model empty on existing tasks); swing_type STOPGAP-falls to heuristic when bronze `stroke_class` absent (swing = next model). Full checklist: `.claude/next_session_pickup.md` §"DEFINITION OF DONE".
...
- **5d — TrackNetV3 retrain. ❌ CLOSED 2026-06-14 — evidence-based NO (don't swap the base tracker).** Architecture ported (`ml_pipeline/tracknet_v3.py`), weights never trained. A best-practice research bench-off (2025 SOTA + our regime) confirmed **WASB + far-ball ROI is already the right foundation; do NOT churn the base tracker before training.** Decisive evidence — RacketVision (AAAI 2026, arXiv 2511.17045) head-to-head on tennis: **WASB ≈ TrackNetV3** (prec 0.937 vs 0.941, mAP 66.0 vs 65.6) → a bare V3 swap buys nothing. The only winner, MS-TrackNetV3 (mAP 81.9), gets its lift from **multi-sport PRO-BROADCAST training data** that does NOT transfer to our amateur side-cam / ~30px far ball (cross-regime transfer fails — confirmed in the lit). TrackNetV5 (Dec 2025) is SOTA but **weights proprietary**; BlurBall is table-tennis-only. Also: the far-ball problem was already solved by the far-ball ROI (40%→87%), the real far-side limiter is **hit/bounce DISCRIMINATION** (a sequence/temporal head, downstream of detection — the actual next lever), and a V3 swap would *worsen* runtime (8-frame + bg + inpaint heavier than WASB; ball isn't the bottleneck). **Only revisit if WASB plateaus on a future bench OR we get broadcast-grade footage.** Optional 1-hr probe if ever wanted: bench MS-TrackNetV3's released checkpoint on our `bench_ball` fixtures — must beat WASB on coverage-gap recall AND ms/frame. Full reasoning in the 2026-06-14 research record.

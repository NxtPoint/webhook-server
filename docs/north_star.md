...
**2026-05-31 — BATCH RUNTIME OPTIMISATION DEPLOYED + PROVEN (rev 59). 4.79h → ~118 min; far-pose cross-fps misalignment FIXED.** Shipped + validated in prod on match 4 (`624e0b36`): player/ROI batching + FP16 + SAHI-batched + MOG2-downscale + CPU/GPU overlap + g5/A10G (main loop 183 → **48 ms/frame**), the **ROI-sweep 25fps-alignment fix** (far-pose was indexed in 60fps source space, misaligned with 25fps bronze on every non-25fps match — now `frame_idx[24..71914]`; also a 2.4× over-decode removed → sweep 91→52 min), pose-fp16-crash + bounce-OOM fixes. **Total 157→118 min, court 88%, 0 errors.** NOT sub-1h yet — main loop (~58 min) + ROI compute remain. **Next = main-loop cycle: `docs/_investigation/t5_runtime_backlog.md` (start B1 — the main VideoPreprocessor over-decodes like the sweep did).** Frame-space gotcha now in MEMORY (`feedback_t5_two_frame_spaces`). [Superseded plan below — kept for the per-stage profile.]

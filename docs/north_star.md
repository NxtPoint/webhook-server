...
**2026-05-27 (late) — DUAL-SUBMIT CORPUS PIPELINE PRODUCTIONISED (the training runway is now hands-off).** Five fixes cleared every blocker to auto-accumulating training corpus from long matches: silver pass-3 hang → route-B staged temp-table (`fdf0697`, 21min→2.1s); sweep self-heals mid-flight ingest deaths (`fa0bba0`); GPU ball-batching activated on Batch (`5317c50`, job-def BALL_BATCH_SIZE=8); ROI single-decode 3→2 (`75c377d`); and the real long-match blocker — T5 in-process bronze ingest OOM'd the 512MB main API (json.loads of the whole 71.7MB export) → now streams via ijson, peak ~15MB (`8dc3b31`). Proven end-to-end: c645a7ee fully recovered (ingest 35653/72565 → serve 261/stroke 597 → silver 371). AUTO_LABEL=1 live. First fully-hands-off run (video 3, SA=2f355924) in flight. **Once corpus auto-lands reliably → bulk-load training videos.** Full detail: `MEMORY.md`→`project_dual_submit_autoland.md`. **NEXT after that: the detector-build phase (one-model-per-fact) — validate serve_detector's silver-inheritance, then build swing-type/bounce/identity/volley; far-court four (far serve, bounce, far-stroke, identity) are coverage+training-gated, not heuristic-buildable.**

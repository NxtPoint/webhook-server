...
def _payload(*, rallies=0, bounces=0, ball=0, players=2, codec="h264", final=0.62):
    return {
        "players": [{"player_id": i} for i in range(players)],
...
CASES = [
    ...
    ("42280d38 74-phantom-players", _payload(rallies=0, bounces=0, ball=0,
                                             players=74, codec="hevc", final=0.451), True, True),
    ("6abd37ca ball-but-no-structure", _payload(rallies=0, bounces=0, ball=496,
                                                players=2, codec="hevc", final=0.408), True, True),
    # --- healthy h264: must pass clean, no warnings ---
    ("e4a74645 healthy", _payload(rallies=101, bounces=1039, ball=34870, players=3, final=0.699), False, False),
    ("299013b3 healthy", _payload(rallies=96, bounces=764, ball=62863, players=3, final=0.627), False, False),
    ("df594aea badly-tracked but REAL", _payload(rallies=9, bounces=982, ball=47754,
                                                 players=3, final=0.618), False, False),
    # --- degraded hevc: warn, but still ingest (they carry some data) ---
    ("f7223270 degraded", _payload(rallies=2, bounces=155, ball=24234,
                                   players=7, codec="hevc", final=0.526), False, True),
    ("36625d04 degraded", _payload(rallies=1, bounces=155, ball=17419,
                                   players=13, codec="hevc", final=0.465), False, True),
]
